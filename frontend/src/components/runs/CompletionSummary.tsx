  ...
  return (
    <div className="space-y-4">
      <div className="completion-hero">
        ...
        <div>
          <p className="kicker">Workflow completed</p>
          <h3 className="mt-1 text-xl font-semibold text-neutral-50">Your repository audit is ready.</h3>
          <p className="mt-2 text-sm leading-6 text-neutral-300">
            FlowPilot completed all {run.summary?.nodes_total ?? run.nodes.length} workflow nodes and generated {run.artifacts.length} deliverables.
          </p>
        </div>
      </div>
      <div className="grid gap-2 sm:grid-cols-2">
        {[
          ["Repo Audit Report", FileText],
          ["README Improvement Plan", FileText],
          ["GitHub Issue Drafts", Github],
          ["LinkedIn Draft", PenLine]
        ].map(([label, Icon]) => (
          <div key={String(label)} className="deliverable-chip">
