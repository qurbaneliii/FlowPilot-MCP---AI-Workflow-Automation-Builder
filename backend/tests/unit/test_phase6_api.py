...
def test_generate_workflow_response_contains_summary_and_node_display(
    client: TestClient,
) -> None:
    body = generate(client)

    assert body["summary"]["name"] == "GitHub Repository Audit"
    assert body["summary"]["node_count"] == len(body["workflow"]["nodes"])
    assert body["summary"]["approval_required"] is True
    assert body["node_display"][0]["id"] == body["workflow"]["nodes"][0]["id"]
    assert {"subtitle", "icon", "order", "stage", "description"} <= set(
        body["node_display"][0]
    )
...
def test_run_response_contains_summary_timeline_and_ui_state(
    client: TestClient,
) -> None:
    workflow_id = generate(client)["workflow_id"]
    run_id = client.post(
    ...
    assert body["summary"]["nodes_total"] == len(body["nodes"])
...
def test_run_response_status_mapping_does_not_mark_pending_issue_creator_completed_before_approval(
    ...
    assert issue_node["status"] == "pending"
...
def test_approve_resumes_run(client: TestClient) -> None:
    ...
    assert body["message"] == "Approval recorded. Workflow will resume."
    assert body["next_poll_recommended"] is True
    assert body["run"]["status"] == "completed"


def test_run_response_completed_recommends_reports_view(client: TestClient) -> None:
    workflow_id = generate(client)["workflow_id"]
    run_id = client.post(
        "/api/v1/workflows/run", json={"workflow_id": workflow_id}
    ).json()["run_id"]
...
def test_node_output_summaries_are_human_readable(client: TestClient) -> None:
    ...
    body = client.get(f"/api/v1/runs/{run_id}").json()
    repo_reader = next(
        item for item in body["node_results"] if item["node_id"] == "github_repo_reader"
    )
...
def test_approve_double_call_controlled(client: TestClient) -> None:
    ...
    assert response.json()["error"]["severity"] == "warning"
...
def test_artifact_tabs_do_not_show_empty_when_artifact_exists(
    client: TestClient,
) -> None:
    workflow_id = generate(client)["workflow_id"]
    run_id = client.post(
    ...
    linkedin = next(
        ...
        if artifact["artifact_type"] == "linkedin_post_draft"
    )

    assert body["artifact_tabs"]["linkedin_post_draft"]["available"] is True
    assert (
        body["artifact_tabs"]["linkedin_post_draft"]["artifact_id"]
        == linkedin["artifact_id"]
    )
...
def test_approval_response_contains_frontend_message_and_poll_hint(
    ...
    body = client.post(f"/api/v1/approvals/{approval_id}/reject", json={}).json()

    assert body["status"] == "rejected"
    assert body["run_id"] == run_id
    assert (
        body["message"] == "Approval rejected. GitHub issue creation will be skipped."
    )
    assert body["next_poll_recommended"] is True
...
def test_api_errors_are_structured(client: TestClient) -> None:
    ...
    assert {"code", "message", "details", "severity", "retryable"} <= set(
        response.json()["error"]
    )


def test_structured_error_contains_severity_and_retryable(client: TestClient) -> None:
    response = client.post(
        "/api/v1/workflows/generate", json={"prompt": "Audit", "repo_url": "nope"}
    )
    body = response.json()["error"]

    assert body["code"] == "INVALID_REPO_URL"
